/* Планка сложности: шкала 0–30, которой человек управляет голосом
 * после занятия или вручную в настройках.
 *
 * Чистый модуль. Про DOM и хранилище не знает.
 *
 * Главное решение: планка не влияет ни на награды, ни на стрик, ни на
 ...
 * планка — текущую настройку, и при смешении низкая планка начнёт
...
export const TIERS = [
  { from: 0,  to: 5,  name: 'Разминка', desc: 'Только самые знакомые слова, выбор из двух вариантов.' },
  { from: 6,  to: 12, name: 'Спокойно', desc: 'Знакомые слова, узнавание и выбор из четырёх.' },
  { from: 13, to: 19, name: 'Рабочая',  desc: 'Добавляются сборка из букв и аудирование.' },
  { from: 20, to: 25, name: 'Плотно',   desc: 'Добавляются ручной ввод и ударения.' },
  { from: 26, to: 30, name: 'Вызов',    desc: 'Ложные друзья, служебные слова, ввод преобладает.' },
];
...
/**
 * Планка задаёт не порог, а полосу в распределении сложности.
...
export const VOTE_REPLY = {
  easy: 'Поднимаю планку. Следующее занятие будет поинтереснее.',
...
export function autoAdjust(ch, stats, day) {
  ...
  // После понижения планка не поднимается автоматически в течение суток:
...
/** Какие типы заданий разрешает текущая планка. */
export function allowedExerciseTypes(index) {
  const types = ['choice2'];
  if (index >= 4) types.push('choice4', 'pair', 'trueFalse');
  if (index >= 10) types.push('reverse4', 'missingLetter', 'audioChoice');
...
export function allowedSources(index) {
  const src = ['deck1'];
  if (index >= 13) src.push('tier34');
  if (index >= 20) src.push('falseFriends');
  if (index >= 22) src.push('deck2');
  return src;
}
