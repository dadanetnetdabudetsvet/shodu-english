...
export class SoundEngine {
  ...
  _play(name, fn) {
    ...
    /* Контекст уже работает — значит _ready отказал не из-за сна, а
       из-за частоты: этот же звук только что играл. Такой звук просто
       пропускается.

       Раньше здесь вызывалось fn(), то есть тот же метод звука. Он
       снова упирался в ограничитель частоты, снова приходил сюда — и
       так до переполнения стека. Ошибка прилетала из нашего кода,
       поэтому сторож честно показывал «Что-то не отозвалось», а
       занятие срывалось: звуки в нём идут на каждый ответ. */
